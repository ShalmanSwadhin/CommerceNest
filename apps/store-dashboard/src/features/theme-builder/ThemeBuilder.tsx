...
import { storeApi, ApiClientError } from '../../lib/api';
...
export function ThemeBuilder({ storeId }: ThemeBuilderProps) {
  ...
  const versionsQ = useQuery({
    queryKey: ['store', storeId, 'theme-versions'],
    queryFn: () => storeApi.themeVersions(storeId),
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      return storeApi.saveThemeDraft(storeId, {
  ...
  const publishMut = useMutation({
    mutationFn: async () => {
      if (dirty && doc) {
        await storeApi.saveThemeDraft(storeId, {
          themeSettings: doc.themeSettings,
          layout: doc.layout,
        });
      }
      return storeApi.publishTheme(storeId);
    },
    onSuccess: () => {
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] min-h-[640px] flex-col bg-surface-raised">
      ...
      <Modal
        ...
        title="Publish theme?"
        description="Your storefront will use this draft. Customers see only published themes."
      >
