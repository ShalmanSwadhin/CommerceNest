...
export function MediaImageField({
  ...
  const mediaQ = useQuery({
    queryKey: ['admin', 'store-media', storeId],
    queryFn: () => adminApi.listStoreMedia(storeId, { limit: 40 }),
  ...
  const registerFile = useCallback(
    async (file: File) => {
      ...
      setBusy(true);
      try {
        const signed = await adminApi.signedStoreUpload(storeId, {
          filename: file.name,
        ...
        const asset = await adminApi.registerStoreMedia(storeId, {
        ...
        onChange(asset.url);
        void qc.invalidateQueries({ queryKey: ['admin', 'store-media', storeId] });
      } catch (err) {
        setError(
